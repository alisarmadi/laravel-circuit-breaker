...
class CircuitBreaker
{
    ...
    public function __construct(string $service, Config $config, Repository $cache) {
    ...
    {
//        dd(! $this->isOpen(), (bool)$this->cache->get(
...
//            ));
    ...
    public function reachedErrorThreshold(): bool
    {
        ...
        return ($failures >= $this->config->getErrorThreshold());
    }
    ...
    public function reachedSuccessThreshold(): bool
    {
        ...
        return ($successes >= $this->config->getSuccessThreshold());
    }
    ...
    protected function incrementErrors(): bool
    {
        ...
        if (! $this->cache->get($key)) {
            return $this->cache->put($key, 1, $this->config->getTimeWindow());
        }
    ...
    protected function setHalfOpenCircuit()
    {
        $this->cache->put(
            ...
            time(),
            $this->config->getErrorTimeout() + $this->config->getHalfOpenTimeout()
        );
//        dd('setHalfOpenCircuit', $this->getKeyName(self::HALF_OPEN),
//            time(),
//            $this->config->getErrorTimeout() + $this->config->getHalfOpenTimeout());
    }
